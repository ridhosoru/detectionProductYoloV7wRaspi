...
class Ui(QtWidgets.QMainWindow):
    ...
    def connect(self,text):
        if text == "0":
            print("detected")
            self.statusdet.setText("detecting ")
            self.capturing()
            self.statusdet.setText("processing")
            print("processing")
            self.processing()
    
    def processing(self):        self.threaddet()
            ...
            if self.cb1.currentText()==nameprod or self.cb2.currentText()==nameprod :
                ...
                if self.cb1.currentText()==nameprod:
                    if count_of_object != 0 :
                        ...
                        time.sleep(0.5)
                        GPIO.output(17,1)
                        QMessageBox.about(self, "warning", "no have product detected")

                    self.qty1.setText(str(count_of_object))
                    self.tp_1.setText(str(y))
                    values=[timenow,str(nameprod),y]
                    c=con.cursor()
                    c.execute("INSERT INTO database(date,product,qty) VALUES(?,?,?)",values)
                ...
                if self.cb2.currentText() == nameprod:
                    ...
                    values=[timenow,str(nameprod),y2]
                    c=con.cursor()
                    c.execute("INSERT INTO database(date,product,qty) VALUES(?,?,?)",values)
                    self.statusdet.setText("done")
                    con.commit()          
