...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.timelbl()
        self.opencamBtn.clicked.connect(self.opcam)
    ...
    def captimgg(self,img,window=1):
        cam = cv2.VideoCapture(0)
        cam.set(cv2.CAP_PROP_FRAME_WIDTH, 320)
        cam.set(cv2.CAP_PROP_FRAME_HEIGHT,320)
        cam.set(cv2.CAP_PROP_FPS,5)
        #cam.set(cv2.CAP_PROP_BRIGHTNESS,255)
        result,imm = cam.read()
        if result == True :
    ...
    def threadopcam(self):
        global tr1
        tr1 = Thread(target = self.opcam)
        tr1.start()
    ...
    def opcam(self):
        ...
        while(cam.isOpened()):
            ...
            if check == True:
                self.displayCam(frame,1)
                cv2.waitKey(0)
        cv2.destroyAllWindows()
        cam.release()

    def displayCam(self,img,window=1):
        qformat = QImage.Format_Indexed8
        if len(img.shape) == 3 :
          if(img.shape[2]) == 4:
            qformat = QImage.Format_RGBA88
          else:
            qformat = QImage.Format_RGB888
        img = QImage(img, img.shape[1],img.shape[0], qformat)
        img = img.rgbSwapped()
        self.lbl_opcam.setPixmap(QPixmap.fromImage(img))
        self.lbl_opcam.setAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
